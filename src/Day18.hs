  ...
  where
    noSoln nbits = null soln
      where
        ...
        soln = Board.shortestPaths board' (0, 0) (nrow - 1, ncol - 1)
...
solve1 :: Solver
solve1 input = show $ length (head paths) - 1 -- count moves, not nodes
  where
    ...
    board = Board.Board nrow ncol (Map.fromList $ map (,()) (take nbits queue))
    paths = Board.shortestPaths board (0, 0) (nrow - 1, ncol - 1)
